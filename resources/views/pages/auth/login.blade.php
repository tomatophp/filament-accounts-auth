    ...
    public function authenticate()
    {
        ...
        if(!$this->showPasswordField){
            ...
            if(config('devdojo.auth.settings.check_account_exists_before_login') && is_null($userTryingToValidate)){
                ...
                $this->addError('email', \Devdojo\Auth\Helper::language('login.couldnt_find_your_account'));
                return;
            }
...
<x-auth::layouts.app title="{{ \Devdojo\Auth\Helper::language('login.page_title') }}">
    ...
    @volt('auth.login')
        <x-auth::elements.container>
            ...
            <form wire:submit="authenticate" class="space-y-5">
                ...
                @if($showPasswordField)
                    <x-auth::elements.input-placeholder value="{{ $email }}">
                        <button type="button" data-auth="edit-email-button" wire:click="editIdentity" class="font-medium text-blue-500">{{ \Devdojo\Auth\Helper::language('login.edit') }}</button>
                    </x-auth::elements.input-placeholder>
                @else
                    @if($showIdentifierInput)
                        <x-auth::elements.input :label="\Devdojo\Auth\Helper::language('login.email_address')" type="email" wire:model="email" autofocus="true" data-auth="email-input" id="email" required />
                    @endif
                @endif

                @if($showSocialProviderInfo)
                    <div class="p-4 text-sm border rounded-md bg-zinc-50 border-zinc-200">
                        <span>{{ str_replace('__social_providers_list__', implode(', ', $userSocialProviders), \Devdojo\Auth\Helper::language('login.social_auth_authenticated_message')) }}</span>
                        <button wire:click="editIdentity" type="button" class="underline translate-x-0.5">{{ \Devdojo\Auth\Helper::language('login.change_email') }}</button>
                    </div>
                ...
                @if($showPasswordField)
                    <x-auth::elements.input :label="\Devdojo\Auth\Helper::language('login.password')" type="password" wire:model="password" id="password" data-auth="password-input" />
					<x-auth::elements.checkbox :label="\Devdojo\Auth\Helper::language('login.remember_me')" wire:model="rememberMe" id="remember-me" data-auth="remember-me-input" />
					<div class="flex items-center justify-between mt-6 text-sm leading-5">
                        <x-auth::elements.text-link href="{{ route('auth.password.request') }}" data-auth="forgot-password-link">{{ \Devdojo\Auth\Helper::language('login.forget_password') }}</x-auth::elements.text-link>
                    </div>
                @endif

                <x-auth::elements.button type="primary" data-auth="submit-button" rounded="md" size="md" submit="true">
                    {{ \Devdojo\Auth\Helper::language('login.button') }}
            ...
            <div class="mt-3 space-x-0.5 text-sm leading-5 text-left" style="color:{{ config('devdojo.auth.appearance.color.text') }}">
                <span class="opacity-[47%]"> {{ \Devdojo\Auth\Helper::language('login.dont_have_an_account') }} </span>
                <x-auth::elements.text-link data-auth="register-link" href="{{ route('auth.register') }}">{{ \Devdojo\Auth\Helper::language('login.sign_up') }}</x-auth::elements.text-link>
            </div>
